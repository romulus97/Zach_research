...
def simulate(vars, 
    ...
    for i in range(0,len(HP)):
        ...
        # Financial exchange
        D = np.float(WP[i]*np.max([0,NP[i]]) - np.max([vars[(month-1)*2+p]-WP[i],0])*NP[i] + (vars[24] - HP[i])*vars[(month-1)*2+p])
        # DeveloperDaily = np.append(DeveloperDaily,D)
        DeveloperProfits += D
        
        T = np.float((HP[i] - vars[24])*vars[(month-1)*2+p])
        TraderRevs += np.max([0,T])
        # TraderProfits += T
        
        DH = np.float((vars[24] - HP[i])*vars[(month-1)*2+p])
    ...
    return [DeveloperProfits, MonthlyVar], Constraints
