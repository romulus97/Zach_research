...
for node in L:
    ...
    node = node #'OKGEKEENANWIND' 'WFECVICILD2'
    
    #define (ranges from 0-99)
    pick = 0 #0 = max floor improvement, 99 = max profits
    
    # objective functions
    filename = '../observed/' + node + '/' + experiment + '/Objective_Functions.csv'
    ...
    filename = '../observed/' + node + '/' + experiment + '/input_data.csv'
    ...
    # hedgetargets
    filename = '../observed/' + node + '/' + experiment + '/Decision_Variables.csv'
    ...
    locals()[ax].plot(np.array(N[2])/1000000,color='black',linewidth=1)
    locals()[ax].plot(np.array(Monthly)/1000000,color='deeppink',linewidth=1.2)
    locals()[ax].plot(np.array(Monthly_P99)/1000000,color='dodgerblue',linewidth=1.2,linestyle='--')
    locals()[ax].set_title(node,fontname='Arial',size=8)
    
    if idx == 8:
        locals()[ax].set_ylim([0,4])

    
    if idx>5:
        locals()[ax].set_xticks([0,12,24,36,48])
        locals()[ax].set_xticklabels(['2015','2016','2017','2018','2019'],size=8,rotation=45)

fig.text(0.05, 0.5, 'Revenues ($ Millions)', fontname='Arial',fontweight='bold', va='center', rotation='vertical',size=12)
fig.text(0.5, 0, 'Year', fontname='Arial',fontweight='bold', va='center',size=12)


plt.subplots_adjust(wspace=.25,hspace=.4)

plt.savefig('figureA1.png',dpi=800)